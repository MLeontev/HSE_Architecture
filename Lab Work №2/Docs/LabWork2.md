...
![alt text](./C4/Components/Frontend/image.png)

### Описание компонентов

1. Маршрутизатор - компонент для управления навигацией в SPA приложении. Определяет, какой компонент страницы отобразить на основе текущего URL

2. API Клиент - модуль для выполнения всех HTTP-запросов к API Backend

3. Страница аутентификации - интерфейс для входа и регистрации пользователей. Отвечает за взаимодействие с аутентификацией через бэкенд

4. Страница собеседования - интерфейс для кандидатов. Отображает задания, редактор кода и отправляет ответы/решения на проверку

5. Страница результатов/отчетов - отображает итоги пройденного собеседования

6. Панель контент-менеджера - интерфейс, доступный контент-менеджеру, для выполнения CRUD-операций с вопросами и задачами
